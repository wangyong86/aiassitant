...
updatedAt: "2026-06-02T22:32:14.923Z"
---

# 需求与设计同步

<!-- message:id=xtool_history role=assistant createdAt=2026-06-02T22%3A32%3A14.923Z -->
...
<!-- /message -->
<!-- message:id=xtool_requirements role=assistant createdAt=2026-06-02T22%3A32%3A14.923Z -->
...
34. Store long-term content as `repo -> notebook -> child notebook -> session`; session transcripts are git-managed content.
35. Store each session transcript in one append-friendly Markdown file named after the session inside the default data repo.
...
38. Follow `docs/development-guide.md`: source and issue tracking are linked to the remote code repository, and every source/documentation/test change must be committed before moving on.
...
- Development workflow rules are documented in `docs/development-guide.md`; every completed source/documentation/test change must be committed.
...
- Deleting one conversation turn from a session should remove the selected user/assistant/error message group from the session Markdown file so future context is shorter. Run metadata and recorded artifacts remain under runtime session storage.
- Child notebooks are limited to one level: root notebook -> child notebook. Deeper nesting is rejected.
- The default repo is git-managed user content. Notebook directories are named with their notebook titles, notebook Markdown files are named with their notebook titles, session directories are named with their session titles, and there is no required `notebooks/` or `sessions/` wrapper directory.
- Session question/answer messages are stored under `/home/wy/ai-server-data/repos/default/<notebook-title>/[<child-notebook-title>/]<session-title>/<session-title>.md`; session metadata is stored under `/home/wy/ai-server-runtime/sessions/<notebook-id>/[<child-notebook-id>/]<session-id>/session.json`, and run metadata is stored under that runtime session directory's `runs/` directory.
...
<!-- /message -->
<!-- message:id=xtool_design role=assistant createdAt=2026-06-02T22%3A32%3A14.923Z -->
  ...
  repos/
    default/
      ...
      <Notebook title>/
        ...
        <Session title>/
          <Session title>.md
        <Child notebook title>/
          <Child notebook title>.md
          <Session title>/
            <Session title>.md
...
- Deleting a session moves only that session directory from `/home/wy/ai-server-runtime/sessions/` to `/home/wy/ai-server-runtime/trash/sessions/`.
...
- `updatedAt`
- notes
- a content Markdown file named after the notebook, such as `Vast/Vast.md`
...
- `updatedAt`
- `settings`
- message transcript Markdown file named after the session, such as `ACL/ACL.md` in the default data repo
- runtime metadata stored as `session.json`
- runtime run records stored in `runs/`

Question/answer storage:

- The default repo stores user-visible notebook structure, notebook notes, and session question/answer transcripts. It is suitable for git sync.
- Notebook directories are named after notebooks. A root notebook named `Vast` with a child notebook named `DataEngine` is stored as `/home/wy/ai-server-data/repos/default/Vast/Vast.md` and `/home/wy/ai-server-data/repos/default/Vast/DataEngine/DataEngine.md`.
- Session directories are named after sessions. A session named `ACL` under child notebook `DataEngine` is stored as `/home/wy/ai-server-data/repos/default/Vast/DataEngine/ACL/ACL.md`.
- If two sibling notebooks have the same title, the later directory gets a numeric suffix such as `Untitled 2`; front matter still preserves the original title.
- If two sibling sessions have the same title, the later directory gets a numeric suffix such as `ACL 2`; front matter still preserves the original title.
- The conversation transcript for a session is stored in the default repo, not under runtime.
- Runtime session metadata for a root notebook session is stored at `/home/wy/ai-server-runtime/sessions/<notebook-id>/<session-id>/session.json`.
- Runtime session metadata for a child notebook session is stored at `/home/wy/ai-server-runtime/sessions/<parent-notebook-id>/<child-notebook-id>/<session-id>/session.json`.
- The Markdown file has YAML-style JSON-valued front matter followed by message blocks marked with HTML comments.
- Run metadata and artifact references are stored under the runtime session directory in `runs/<run-id>.json`.

Child notebooks:

- Only one child level is supported.
- Child notebook directories live directly under their parent notebook directory in the content repo.
